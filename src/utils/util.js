...
function generateChildRouters(data) {
  ...
  for (let item of data) {
    ...
    if (item.component == 'modules/online/cgform/OnlCgformHeadList') {
      ...
      item.component == 'modules/online/cgform/auto/OnlCgformAutoList'
    ) {
      componentPath = onlineCommons.OnlCgformAutoList
    } else if (
      item.component == 'modules/online/cgform/auto/OnlCgformTreeList'
    ) {
      componentPath = onlineCommons.OnlCgformTreeList
    } else if (
      item.component == 'modules/online/cgform/auto/erp/OnlCgformErpList'
    ) {
      componentPath = onlineCommons.OnlCgformErpList
    } else if (
      item.component == 'modules/online/cgform/auto/tab/OnlCgformTabList'
    ) {
      ...
      item.component == 'modules/online/cgreport/OnlCgreportHeadList'
    ) {
      componentPath = onlineCommons.OnlCgreportHeadList
    } else if (
      item.component == 'modules/online/cgreport/auto/OnlCgreportAutoList'
    ) {
 ...
 * 单个工单查询，现场情况
 *
 *  @params liveVideo现场视频
 *  @params livePhotos现场照片
 *  @params livePhotos现场照片
 *  @params livePhotos现场照片
 *  @params livePhotos现场照片
 */

export async function dealSituation(
  data,
  liveVideo,
  livePhotos,
  liveSituation,
  ...
  if (data != null) {
    if (data[liveVideo] != null && data[liveVideo].length > 0) {
      item.liveVideo = [data[liveVideo]]
    }
    if (data[livePhotos] != null && data[livePhotos].length > 0) {
      let photoList = data[livePhotos].split(',').filter(i => i.length > 0)
      for (let i = 0; i < photoList.length; i++) {
        const res = await postAction(`file/getDown?uniqueId=${photoList[i]}`)
        item.livePhotos.push(`data:image/png;base64,${res.base64}`)
      }
      // data[livePhotos].split(",").map((i) => {
      //   if (i) {
      //     item.livePhotos.push(i);
      //   }
      // });
    }
    item.pStatus = ''
