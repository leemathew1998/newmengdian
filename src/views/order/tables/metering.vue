...
import {
  ListOfReason
} from '@/views/order/utils/MeteringEnum.js'
const columns = [{
  title: '工单编号',
...
export default {
  ...
  methods: {
    ...
    async clickRows (e) {
      ...
      const res = dealSituation(temp, 'liveVideo', 'livePhotos', 'liveSituation')
      ...
      // 显示原因
      const reason = await postAction(`mwo/selectOne?workOrderNo=${this.clickRow.workOrderNo}`)
      for (let i = 0; i < ListOfReason.length; i++) {
        if (reason[ListOfReason[i].englishName] != null && !this.isNeedObj(i, reason[ListOfReason[i].englishName])) {
          if (ListOfReason[i].englishName == 'ninesix1' || ListOfReason[i].englishName == 'ninesix2') {
            this.dictionary.push({
              label: '时间',
              name: ListOfReason[i].englishName,
            })
            this.NewModelData[ListOfReason[i].englishName] =
              `${moment().subtract(1, 'day').format('MM月DD日')}${reason[ListOfReason[i].englishName].split('时')[0]}分`
            ...
            this.NewModelData[ListOfReason[i].englishName] = reason[ListOfReason[i].englishName]
            ...
            this.NewModelData[ListOfReason[i].englishName] = reason[ListOfReason[i].englishName]
          } else if (ListOfReason[i].englishName == 'nowPositiveActive' || ListOfReason[i].englishName ==
            'nowavgCurrentA' ||
            ListOfReason[i].englishName == 'nowavgCurrentB' || ListOfReason[i].englishName ==
            'nowavgCurrentC'
          ) {
            this.dictionary.push({
              label: `${moment().subtract(1, 'day').format('MM月DD日')}${ListOfReason[i].chineseName}`,
            ...
            this.NewModelData[ListOfReason[i].englishName] = reason[ListOfReason[i].englishName]
          } else if (ListOfReason[i].englishName == 'beforePositiveActive' || ListOfReason[i]
            .englishName == 'beforeavgCurrentA' ||
            ListOfReason[i].englishName == 'beforeavgCurrentB' || ListOfReason[i].englishName ==
            'beforeavgCurrentC'
