  ...
  methods: {
    ...
    async loadData () {
      ...
      for (var i = 0; i < this.data.length; i++) {
        ...
        if (this.data[i].workOrderStatus == 'STATUS_ONE') {
          this.data[i].workOrderStatus = '待处理'
        } else if (this.data[i].workOrderStatus == 'STATUS_TWO') {
          this.data[i].workOrderStatus = '处理中'
        } else if (this.data[i].workOrderStatus == 'STATUS_THREE') {
          this.data[i].workOrderStatus = '待归档'
        } else if (this.data[i].workOrderStatus == 'STATUS_FOUR') {
    ...
    async clickRows (e) {
      ...
      const { data } = await getAction(
        'feecontrolWorkOrder/selectWorkOrderScene?workOrderNo=' +
        e.workOrderNo
      )
