    ...
    >
      ...
      <div class="record">
        ...
        <div
          ...
          v-if="
            newModelData.contactNames && newModelData.contactNames.length > 0
          "
        >
          <div
            class="contact-item"
            v-for="(name, index) in newModelData.contactNames"
            :key="index"
          >
            <a-tooltip>
              <template slot="title"> 联系人：{{ name }} </template>
              <span>联系人：{{ name }}</span>
            </a-tooltip>
            <span>联系类型：{{ newModelData["contactModes"][index] }}</span>
            <span>移动电话：{{ newModelData["mobiles"][index] }}</span>
          </div>
        </div>
      </div>
    </a-modal>
...
export default {
  ...
  methods: {
    ...
    async pilsonFormula() {
      ...
      const res1 = await postAction(
        '/lineloss/pearson?tgId=' + this.newModelData.tgId
      )
    ...
    async timeCalculation() {
      ...
      const res = await postAction(
        '/lineloss/split?tgId=' + this.newModelData.tgId
      )
